...
char *disknet_grub_net_default_server;

struct disknet_grub_net_route *disknet_grub_net_routes = NULL;
struct disknet_grub_net_network_level_interface *disknet_grub_net_network_level_interfaces = NULL;
struct disknet_grub_net_card *disknet_grub_net_cards = NULL;
struct disknet_grub_net_network_level_protocol *disknet_grub_net_network_level_protocols = NULL;
// static struct grub_fs disknet_grub_net_fs;

struct disknet_grub_net_link_layer_entry {
  int avail;
  disknet_grub_net_network_level_address_t nl_address;
  disknet_grub_net_link_level_address_t ll_address;
};

#define LINK_LAYER_CACHE_SIZE 256

static struct disknet_grub_net_link_layer_entry *
link_layer_find_entry (const disknet_grub_net_network_level_address_t *proto,
		       const struct disknet_grub_net_card *card)
{
  unsigned i;
  ...
  for (i = 0; i < LINK_LAYER_CACHE_SIZE; i++)
    {
	  ...
	  && disknet_grub_net_addr_cmp (&card->link_layer_table[i].nl_address,
				proto) == 0)
...
disknet_grub_net_link_layer_add_address (struct disknet_grub_net_card *card,
				 ...
				 int override)
{
  struct disknet_grub_net_link_layer_entry *entry;
				   ...
				   const disknet_grub_net_network_level_address_t *proto_addr)
{
  ...
  if (proto_addr->type == disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV4
			     ...
			     disknet_grub_net_link_level_address_t *hw_addr)
{
  ...
  if ((proto_addr->type == disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV4
       && proto_addr->ipv4 == 0xffffffff)
      || proto_addr->type == disknet_grub_net_NETWORK_LEVEL_PROTOCOL_DHCP_RECV
      || (proto_addr->type == disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV6
	  && proto_addr->ipv6[0] == grub_be_to_cpu64_compile_time (0xff02ULL
								   << 48)
	  && proto_addr->ipv6[1] == (grub_be_to_cpu64_compile_time (1))))
    {
      hw_addr->type = disknet_grub_net_LINK_LEVEL_PROTOCOL_ETHERNET;
      grub_memset (hw_addr->mac, -1, 6);
      return GRUB_ERR_NONE;
    }

  if (proto_addr->type == disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV6
  ...
  switch (proto_addr->type)
    ...
    case disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV4:
      err = disknet_grub_net_arp_send_request (inf, proto_addr);
      break;
    case disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV6:
      err = disknet_grub_net_icmp6_send_request (inf, proto_addr);
      break;
    case disknet_grub_net_NETWORK_LEVEL_PROTOCOL_DHCP_RECV:
...
{
  struct disknet_grub_net_network_level_interface *inf, *next;
  FOR_NET_NETWORK_LEVEL_INTERFACES_SAFE(inf, next)
    if (inf->card == card)
      disknet_grub_net_network_level_interface_unregister (inf);
  if (card->opened)
    {
...
// static struct disknet_grub_net_slaac_mac_list *
// disknet_grub_net_ipv6_get_slaac (struct disknet_grub_net_card *card,
...
//     if (disknet_grub_net_hwaddr_cmp (&slaac->address, hwaddr) == 0)
//       return slaac;
...
// 			     + disknet_grub_net_MAX_STR_HWADDR_LEN
// 			     + sizeof (":slaac"));
//   ptr = grub_stpcpy (slaac->name, card->name);
//   if (disknet_grub_net_hwaddr_cmp (&card->default_address, hwaddr) != 0)
//     {
//       ptr = grub_stpcpy (ptr, ":");
//       disknet_grub_net_hwaddr_to_str (hwaddr, ptr);
//       ptr += grub_strlen (ptr);
//     }
...
static void
disknet_grub_net_network_level_interface_register (struct disknet_grub_net_network_level_interface *inter);

static struct disknet_grub_net_network_level_interface *
disknet_grub_net_add_addr_real (char *name,
			...
			disknet_grub_net_interface_flags_t flags)
{
  ...
  disknet_grub_net_network_level_interface_register (inter);
		   ...
		   disknet_grub_net_interface_flags_t flags)
{
  ...
  struct disknet_grub_net_network_level_interface *ret;

  if (!name_dup)
    return NULL;
  ret = disknet_grub_net_add_addr_real (name_dup, card, addr, hwaddress, flags);
  if (!ret)
    grub_free (name_dup);
  return ret;
}
			      ...
			      const disknet_grub_net_link_level_address_t *hwaddr)
{
  ...
  addr.ipv6[1] = disknet_grub_net_ipv6_get_id (hwaddr);

  FOR_NET_NETWORK_LEVEL_INTERFACES (inf)
  {
    if (inf->card == card
	&& disknet_grub_net_hwaddr_cmp (&inf->hwaddress, hwaddr) == 0
  ...
  return disknet_grub_net_add_addr_real (name, card, &addr, hwaddr, 0);
}
...
// {
//   struct disknet_grub_net_card *card;
//   struct disknet_grub_net_network_level_interface **ifaces;
//   grub_size_t ncards = 0;
//   unsigned j = 0;
//   int interval;
//   grub_err_t err;
//   struct disknet_grub_net_slaac_mac_list **slaacs;
...
//     if (argc > 0 && grub_strcmp (card->name, args[0]) != 0)
//       continue;
//     ifaces[j] = disknet_grub_net_ipv6_get_link_local (card, &card->default_address);
//     if (!ifaces[j])
//       {
// 	grub_free (ifaces);
// 	grub_free (slaacs);
// 	return grub_errno;
//       }
//     slaacs[j] = disknet_grub_net_ipv6_get_slaac (card, &card->default_address);
//     if (!slaacs[j])
//       {
// 	grub_free (ifaces);
// 	grub_free (slaacs);
...
// 	    continue;
// 	  err = disknet_grub_net_icmp6_send_router_solicit (ifaces[j]);
// 	  if (err)
// 	    err = GRUB_ERR_NONE;
// 	  done = 0;
// 	}
//       if (done)
// 	break;
//       disknet_grub_net_poll_cards (interval, 0);
//     }
...
static int
match_net (const disknet_grub_net_network_level_netaddress_t *net,
	   const disknet_grub_net_network_level_address_t *addr)
{
			  ...
			  disknet_grub_net_network_level_address_t *addr)
{
  ...
  struct disknet_grub_net_network_level_address *addresses = 0;
  if (parse_ip (name, &addr->ipv4, &rest) && *rest == 0)
    {
      addr->type = disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV4;
      return GRUB_ERR_NONE;
    }
  ...
  err = disknet_grub_net_dns_lookup (name, 0, 0, &naddresses, &addresses, 1);
  if (err)
    return err;
  if (!naddresses)
...
static int
route_cmp (const struct disknet_grub_net_route *a, const struct disknet_grub_net_route *b)
{
  if (a == NULL && b == NULL)
    return 0;
  if (b == NULL)
			...
			struct disknet_grub_net_network_level_interface **interf)
{
  ...
  struct disknet_grub_net_network_level_protocol *prot = NULL;
  disknet_grub_net_network_level_address_t curtarget = addr;
  ...
  *gateway = addr;
  ...
  for (depth = 0; depth < routecnt + 2 && depth < GRUB_UINT_MAX; depth++)
    {
      struct disknet_grub_net_route *bestroute = NULL;
      FOR_NET_ROUTES(route)
      {
...
//   if (inter->flags & disknet_grub_net_INTERFACE_PERMANENT)
//     return grub_error (GRUB_ERR_IO,
// 		       N_("you can't delete this address"));

//   disknet_grub_net_network_level_interface_unregister (inter);
...
void
disknet_grub_net_addr_to_str (const disknet_grub_net_network_level_address_t *target, char *buf)
{
  switch (target->type)
    {
    case disknet_grub_net_NETWORK_LEVEL_PROTOCOL_DHCP_RECV:
      COMPILE_TIME_ASSERT (sizeof ("temporary") < disknet_grub_net_MAX_STR_ADDR_LEN);
	...
	grub_snprintf (buf, disknet_grub_net_MAX_STR_ADDR_LEN, "%d.%d.%d.%d",
		       ((n >> 24) & 0xff), ((n >> 16) & 0xff),
		       ((n >> 8) & 0xff), ((n >> 0) & 0xff));
      }
      return;
    }
  grub_snprintf (buf, disknet_grub_net_MAX_STR_ADDR_LEN,
...
disknet_grub_net_hwaddr_to_str (const disknet_grub_net_link_level_address_t *addr, char *str)
{
  ...
  switch (addr->type)
    {
    case disknet_grub_net_LINK_LEVEL_PROTOCOL_ETHERNET:
      {
	char *ptr;
	unsigned i;
	for (ptr = str, i = 0; i < ARRAY_SIZE (addr->mac); i++)
	  {
	    grub_snprintf (ptr, disknet_grub_net_MAX_STR_HWADDR_LEN - (ptr - str),
			   "%02x:", addr->mac[i] & 0xff);
	    ptr += (sizeof ("XX:") - 1);
	  }
      return;
...
// 		   const char *val)
// {
//   grub_free (disknet_grub_net_default_server);
//   disknet_grub_net_default_server = grub_strdup (val);
...
// 		   const char *val __attribute__ ((unused)))
// {
//   return disknet_grub_net_default_server ? : "";
// }
...
{
  {
    char buf[disknet_grub_net_MAX_STR_HWADDR_LEN];
    char *name;
    char *ptr;
    disknet_grub_net_hwaddr_to_str (&inter->hwaddress, buf);
  ...
  {
    char buf[disknet_grub_net_MAX_STR_ADDR_LEN];
    char *name;
    char *ptr;
    disknet_grub_net_addr_to_str (&inter->address, buf);
  ...
  inter->prev = &disknet_grub_net_network_level_interfaces;
  inter->next = disknet_grub_net_network_level_interfaces;
  if (inter->next)
    inter->next->prev = &inter->next;
  disknet_grub_net_network_level_interfaces = inter;
}


grub_err_t
disknet_grub_net_add_ipv4_local (struct disknet_grub_net_network_level_interface *inter,
			 int mask)
{
  grub_uint32_t ip_cpu;
  struct disknet_grub_net_route *route;

  if (inter->address.type != disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV4)
    return 0;
...
//   disknet_grub_net_network_level_address_t addr;
//   grub_err_t err;
//   disknet_grub_net_interface_flags_t flags = 0;
//   struct disknet_grub_net_network_level_interface *inf;
...
//   err = disknet_grub_net_resolve_address (args[2], &addr);
//   if (err)
//     return err;

//   if (card->flags & disknet_grub_net_CARD_NO_MANUAL_INTERFACES)
//     return grub_error (GRUB_ERR_IO,
// 		       "this card doesn't support address addition");

//   if (card->flags & disknet_grub_net_CARD_HWADDRESS_IMMUTABLE)
//     flags |= disknet_grub_net_INTERFACE_HWADDRESS_IMMUTABLE;

//   inf = disknet_grub_net_add_addr (args[0], card, &addr, &card->default_address,
// 			   flags);
//   if (inf)
//     disknet_grub_net_add_ipv4_local (inf, -1);
...
//   for (prev = &disknet_grub_net_routes, route = *prev; route; prev = &((*prev)->next),
// 	 route = *prev)
...
//   disknet_grub_net_resolve_net_address  (args[1], &target);
...
//       err = disknet_grub_net_resolve_address (args[3], &gw);
//       if (err)
// 	return err;
//       return disknet_grub_net_add_route_gw (args[0], target, gw, NULL);
//     }
//   else
//     {
//       struct disknet_grub_net_network_level_interface *inter;
...
//       return disknet_grub_net_add_route (args[0], target, inter);
//     }
// }

// static void
// print_net_address (const disknet_grub_net_network_level_netaddress_t *target)
// {
//   switch (target->type)
//     {
//     case disknet_grub_net_NETWORK_LEVEL_PROTOCOL_DHCP_RECV:
...
//       {
// 	char buf[disknet_grub_net_MAX_STR_ADDR_LEN];
// 	struct disknet_grub_net_network_level_address base;
// 	base.type = disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV6;
// 	grub_memcpy (&base.ipv6, &target->ipv6, 16);
// 	disknet_grub_net_addr_to_str (&base, buf);
...
// static void
// print_address (const disknet_grub_net_network_level_address_t *target)
// {
//   char buf[disknet_grub_net_MAX_STR_ADDR_LEN];
//   disknet_grub_net_addr_to_str (target, buf);
//   grub_xputs (buf);
// }
...
//   {
//     char buf[disknet_grub_net_MAX_STR_HWADDR_LEN];
//     disknet_grub_net_hwaddr_to_str (&card->default_address, buf);
//     grub_printf ("%s %s\n", card->name, buf);
//   }
//   return GRUB_ERR_NONE;
// }
...
//   FOR_NET_NETWORK_LEVEL_INTERFACES (inf)
//   {
//     char bufh[disknet_grub_net_MAX_STR_HWADDR_LEN];
//     char bufn[disknet_grub_net_MAX_STR_ADDR_LEN];
//     disknet_grub_net_hwaddr_to_str (&inf->hwaddress, bufh);
//     disknet_grub_net_addr_to_str (&inf->address, bufn);
//     grub_printf ("%s %s %s\n", inf->name, bufh, bufn);
//   }
//   return GRUB_ERR_NONE;
// }

disknet_grub_net_app_level_t disknet_grub_net_app_level_list;
struct disknet_grub_net_socket *disknet_grub_net_sockets;

// static disknet_grub_net_t
// disknet_grub_net_open_real (const char *name)
// {
//   disknet_grub_net_app_level_t proto;
...
//       protname = "tftp";
//       protnamelen = sizeof ("tftp") - 1;
//       server = disknet_grub_net_default_server;
//     }
//   else
//     {
//       const char *comma;
...
// 	  {
// 	    disknet_grub_net_t ret = grub_zalloc (sizeof (*ret));
// 	    if (!ret)
// 	      return NULL;
// 	    ret->protocol = proto;
...
// static grub_err_t
// disknet_grub_net_fs_dir (grub_device_t device, const char *path __attribute__ ((unused)),
...
//   file->device->disknet->packs.first = NULL;
//   file->device->disknet->packs.last = NULL;
//   file->device->disknet->name = grub_strdup (name);
//   if (!file->device->disknet->name)
//     {
//       grub_free (file);
//       return grub_errno;
//     }

//   err = file->device->disknet->protocol->open (file, name);
//   if (err)
//     {
...
// 	}
//       grub_free (file->device->disknet->name);
//       grub_free (file);
//       return err;
//     }
...
// 	  disknet_grub_net_remove_packet (file->device->disknet->packs.first);
// 	}
//       file->device->disknet->protocol->close (file);
//       grub_free (file->device->disknet->name);
//       grub_free (file);
//       return grub_errno;
//     }
...
// static grub_err_t
// disknet_grub_net_fs_close (grub_file_t file)
// {
//   // while (file->device->net->packs.first)
//   //   {
//   //     disknet_grub_netbuff_free (file->device->net->packs.first->nb);
//   //     disknet_grub_net_remove_packet (file->device->net->packs.first);
//   //   }
...
static void
receive_packets (struct disknet_grub_net_card *card, int *stop_condition)
{
  int received = 0;
  ...
  while (received < 100)
      ...
      if (!nb)
	{
      ...
      received++;
      grub_errno = disknet_grub_net_recv_ethernet_packet (nb, card);
      if (grub_errno)
	{
...
// static void
// disknet_grub_net_poll_cards_idle_real (void)
// {
//   struct disknet_grub_net_card *card;
//   FOR_NET_CARDS (card)
//   {
...
// static grub_ssize_t
// disknet_grub_net_fs_read_real (grub_file_t file, char *buf, grub_size_t len)
// {
//   disknet_grub_net_t net = file->device->net;
//   struct disknet_grub_net_buff *nb;
//   char *ptr = buf;
//   grub_size_t amount, total = 0;
//   int try = 0;

//   while (try <= disknet_grub_net_TRIES)
//     {
...
// 	      disknet_grub_net_remove_packet (net->packs.first);
// 	    }
// 	  else
// 	    nb->data += amount;
...
// 	{
// 	  try++;
// 	  disknet_grub_net_poll_cards (disknet_grub_net_INTERVAL +
//                                (try * disknet_grub_net_INTERVAL_ADDITION), &net->stall);
//         }
//       else
// 	return total;
//     }
...
// static grub_err_t
// disknet_grub_net_seek_real (struct grub_file *file, grub_off_t offset)
// {
//   return GRUB_ERR_NONE;
  ...
  //     if (!file->device->net->protocol->seek || have_ahead (file) >= offset)
	// {
	//   disknet_grub_net_fs_read_real (file, NULL,
  ...
  //   if (err)
  //     return err;
  //   disknet_grub_net_fs_read_real (file, NULL, offset);
  //   return grub_errno;
  // }
//}

// static grub_ssize_t
// disknet_grub_net_fs_read (grub_file_t file, char *buf, grub_size_t len)
// {
  // if (file->offset != file->device->net->offset)
  //   {
  //     grub_err_t err;
  //     err = disknet_grub_net_seek_real (file, file->offset);
  //     if (err)
	// return err;
  //   }
//   return disknet_grub_net_fs_read_real (file, buf, len);
// }

// static struct grub_fs disknet_grub_net_fs =
//   {
//     .name = "netfs",
//     .fs_dir = disknet_grub_net_fs_dir,
//     .fs_open = disknet_grub_net_fs_open,
//     .fs_read = disknet_grub_net_fs_read,
//     .fs_close = disknet_grub_net_fs_close,
//     .fs_label = NULL,
//     .fs_uuid = NULL,
//     .fs_mtime = NULL,
//   };
...
// static grub_err_t
// disknet_grub_net_fini_hw (int noreturn __attribute__ ((unused)))
// {
//   struct disknet_grub_net_card *card;
//   FOR_NET_CARDS (card)
//     if (card->opened)
...
// static grub_err_t
// disknet_grub_net_restore_hw (void)
// {
//   return GRUB_ERR_NONE;
// }
  ...
  FOR_NET_NETWORK_LEVEL_INTERFACES (inf)
    {
      ...
      disknet_grub_net_hwaddr_to_str (&inf->hwaddress, suffix + 3);
        ...
        case disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV4:
          {
            ...
            grub_snprintf (suffix, disknet_grub_net_MAX_STR_ADDR_LEN, "%02X%02X%02X%02X", \
                           ((n >> 24) & 0xff), ((n >> 16) & 0xff),      \
                           ((n >> 8) & 0xff), ((n >> 0) & 0xff));
        ...
        case disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV6:
          {
            ...
            grub_snprintf (suffix, disknet_grub_net_MAX_STR_ADDR_LEN, "%s", buf);
            if (grub_config_search_through (config, suffix, 1, 0) == 0)
              return GRUB_ERR_NONE;
            break;
          }
