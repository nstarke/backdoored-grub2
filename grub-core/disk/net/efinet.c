...
static grub_err_t
send_card_buffer (struct disknet_grub_net_card *dev,
		  struct disknet_grub_net_buff *pack)
{
...
static struct disknet_grub_net_buff *
get_card_packet (struct disknet_grub_net_card *dev)
{
  grub_efi_simple_network_t *net = dev->efi_net;
  grub_err_t err;
  grub_efi_status_t st;
  ...
  nb = disknet_grub_netbuff_alloc (bufsize + 2);
  if (!nb)
    return NULL;

  /* Reserve 2 bytes so that 2 + 14/18 bytes of ethernet header is divisible
     by 4. So that IP header is aligned on 4 bytes. */
  if (disknet_grub_netbuff_reserve (nb, 2))
    {
      disknet_grub_netbuff_free (nb);
      return NULL;
    }
  grub_memcpy (nb->data, dev->rcvbuf, bufsize);
  err = disknet_grub_netbuff_put (nb, bufsize);
  if (err)
    {
      disknet_grub_netbuff_free (nb);
      return NULL;
...
static grub_err_t
open_card (struct disknet_grub_net_card *dev)
{
...
static void
close_card (struct disknet_grub_net_card *dev)
{
...
static struct disknet_grub_net_card_driver efidriver =
  {
    .name = "efinet",
    .open = open_card,
    .close = close_card,
...
// grub_efi_handle_t
// disknet_grub_efinet_get_device_handle (struct disknet_grub_net_card *card)
// {
...
void
disknet_grub_efinet_findcards (void)
{
  ...
  for (handle = handles; num_handles--; handle++)
	...
	continue;
      ...
      card = grub_zalloc (sizeof (struct disknet_grub_net_card));
      if (!card)
	{
	  grub_print_error ();
	  grub_free (handles);
      ...
      if (!card->txbuf)
	{
      ...
      card->flags = 0;
      card->default_address.type = disknet_grub_net_LINK_LEVEL_PROTOCOL_ETHERNET;
      ...
      disknet_grub_net_card_register (card);
    }
  grub_free (handles);
}
...
//       continue;
//     pxe_mode = pxe->mode;
//     disknet_grub_net_configure_by_dhcp_ack (card->name, card, 0,
