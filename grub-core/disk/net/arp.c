...
enum
  {
    ...
    disknet_grub_net_ARPHRD_ETHERNET = 1
  };
			   ...
			   const disknet_grub_net_network_level_address_t *proto_addr)
{
  ...
  disknet_grub_net_link_level_address_t target_mac_addr;
  grub_err_t err;
  int i;
  grub_uint8_t *nbd;
  ...
  disknet_grub_netbuff_clear (&nb);
  disknet_grub_netbuff_reserve (&nb, 128);

  err = disknet_grub_netbuff_push (&nb, sizeof (*arp_packet));
  if (err)
    return err;

  arp_packet = (struct arppkt *) nb.data;
  arp_packet->hrd = grub_cpu_to_be16_compile_time (disknet_grub_net_ARPHRD_ETHERNET);
  arp_packet->hln = 6;
  arp_packet->pro = grub_cpu_to_be16_compile_time (disknet_grub_net_ETHERTYPE_IP);
  arp_packet->pln = 4;
  ...
  nbd = nb.data;
  disknet_send_ethernet_packet (inf, &nb, target_mac_addr, disknet_grub_net_ETHERTYPE_ARP);
...
grub_err_t
disknet_grub_net_arp_receive (struct disknet_grub_net_buff *nb, struct disknet_grub_net_card *card,
                      grub_uint16_t *vlantag)
{
  struct arppkt *arp_packet = (struct arppkt *) nb->data;
  disknet_grub_net_network_level_address_t sender_addr, target_addr;
  disknet_grub_net_link_level_address_t sender_mac_addr;
  struct disknet_grub_net_network_level_interface *inf;

  if (arp_packet->pro != grub_cpu_to_be16_compile_time (disknet_grub_net_ETHERTYPE_IP)
      || arp_packet->pln != 4 || arp_packet->hln != 6
      || nb->tail - nb->data < (int) sizeof (*arp_packet))
    return GRUB_ERR_NONE;

  sender_addr.type = disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV4;
  target_addr.type = disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV4;
  sender_addr.ipv4 = arp_packet->sender_ip;
  target_addr.ipv4 = arp_packet->recv_ip;
  if (arp_packet->sender_ip == pending_req)
    have_pending = 1;

  sender_mac_addr.type = disknet_grub_net_LINK_LEVEL_PROTOCOL_ETHERNET;
  grub_memcpy (sender_mac_addr.mac, arp_packet->sender_mac,
	       sizeof (sender_mac_addr.mac));
  disknet_grub_net_link_layer_add_address (card, &sender_addr, &sender_mac_addr, 1);
  ...
  FOR_NET_NETWORK_LEVEL_INTERFACES (inf)
  {
    ...
    if (disknet_grub_net_addr_cmp (&inf->address, &target_addr) == 0
	&& arp_packet->op == grub_cpu_to_be16_compile_time (ARP_REQUEST))
      {
	disknet_grub_net_link_level_address_t target;
	...
	disknet_grub_netbuff_clear (&nb_reply);
	disknet_grub_netbuff_reserve (&nb_reply, 128);

	err = disknet_grub_netbuff_push (&nb_reply, sizeof (*arp_packet));
	if (err)
	  return err;

	arp_reply = (struct arppkt *) nb_reply.data;

	arp_reply->hrd = grub_cpu_to_be16_compile_time (disknet_grub_net_ARPHRD_ETHERNET);
	arp_reply->pro = grub_cpu_to_be16_compile_time (disknet_grub_net_ETHERTYPE_IP);
	arp_reply->pln = 4;
	arp_reply->hln = 6;
	...
	target.type = disknet_grub_net_LINK_LEVEL_PROTOCOL_ETHERNET;
	...
	disknet_send_ethernet_packet (inf, &nb_reply, target, disknet_grub_net_ETHERTYPE_ARP);
      }
  }
  return GRUB_ERR_NONE;
}
