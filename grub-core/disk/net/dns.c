...
typedef enum disknet_grub_dns_qtype_id
  {
    GRUB_DNS_QTYPE_A = 1,
    GRUB_DNS_QTYPE_AAAA = 28
  } disknet_grub_dns_qtype_id_t;

static struct dns_cache_element dns_cache[DNS_CACHE_SIZE];
static struct disknet_grub_net_network_level_address *dns_servers;
  ...
  if (dns_servers_alloc <= dns_nservers)
    {
      ...
      struct disknet_grub_net_network_level_address *ns;
      grub_size_t sz;
...
disknet_grub_net_remove_dns_server (const struct disknet_grub_net_network_level_address *s)
{
  ...
  for (i = 0; i < dns_nservers; i++)
    if (disknet_grub_net_addr_cmp (s, &dns_servers[i]) == 0)
      break;
  if (i < dns_nservers)
    {
...
static grub_err_t
recv_hook (disknet_grub_net_udp_socket_t sock __attribute__ ((unused)),
		     ...
		     int cache)
{
  ...
  grub_size_t i, j;
  struct disknet_grub_net_buff *nb;
  disknet_grub_net_udp_socket_t *sockets;
  grub_uint8_t *optr;
  const char *iptr;
  struct dns_header *head;
  ...
  *naddresses = 0;
  ...
  nb = disknet_grub_netbuff_alloc (disknet_grub_net_OUR_MAX_IP_HEADER_SIZE
			   + disknet_grub_net_MAX_LINK_HEADER_SIZE
			   + disknet_grub_net_UDP_HEADER_SIZE
  ...
  disknet_grub_netbuff_reserve (nb, disknet_grub_net_OUR_MAX_IP_HEADER_SIZE
			+ disknet_grub_net_MAX_LINK_HEADER_SIZE
			+ disknet_grub_net_UDP_HEADER_SIZE);
  disknet_grub_netbuff_put (nb, sizeof (struct dns_header)
		    + grub_strlen (name) + 2 + 4);
  head = (struct dns_header *) nb->data;
  ...
  *optr++ = 1;
  ...
  for (i = 0; i < n_servers * 4; i++)
	...
	{
	  sockets[send_servers] = disknet_grub_net_udp_open (servers[try_server++],
						     DNS_PORT,
						     recv_hook,
						     &data);
      ...
      for (j = 0; j < send_servers; j++)
	{
          ...
          do
            {
              nb->data = nbd;
              if (servers[j].option == DISKNET_DNS_OPTION_IPV4 ||
                 ((servers[j].option == DISKNET_DNS_OPTION_PREFER_IPV4) && (t++ == 0)) ||
                 ((servers[j].option == DISKNET_DNS_OPTION_PREFER_IPV6) && (t++ == 1)))
              ...
              err2 = disknet_grub_net_send_udp_packet (sockets[j], nb);
              if (err2)
                {
                  grub_errno = GRUB_ERR_NONE;
                  err = err2;
      ...
      disknet_grub_net_poll_cards (200, &data.stop);
    }
 out:
  grub_free (data.name);
  disknet_grub_netbuff_free (nb);
  for (j = 0; j < send_servers; j++)
    disknet_grub_net_udp_close (sockets[j]);
...
// {
//   grub_err_t err;
//   struct disknet_grub_net_network_level_address cmd_server;
//   struct disknet_grub_net_network_level_address *servers;
...
//   if (argc == 2)
//     {
//       err = disknet_grub_net_resolve_address (args[1], &cmd_server);
//       if (err)
// 	return err;
//       servers = &cmd_server;
//       nservers = 1;
...
//   disknet_grub_net_dns_lookup (args[0], servers, nservers, &naddresses,
//                        &addresses, 0);

//   for (i = 0; i < naddresses; i++)
//     {
//       char buf[disknet_grub_net_MAX_STR_ADDR_LEN];
//       disknet_grub_net_addr_to_str (&addresses[i], buf);
//       grub_printf ("%s\n", buf);
//     }
//   grub_free (addresses);
//   if (naddresses)
...
//         {
//         case DISKNET_DNS_OPTION_IPV4:
//           strtype = _("only ipv4");
//           break;

//         case DISKNET_DNS_OPTION_IPV6:
//           strtype = _("only ipv6");
//           break;

//         case DISKNET_DNS_OPTION_PREFER_IPV4:
//           strtype = _("prefer ipv4");
//           break;

//         case DISKNET_DNS_OPTION_PREFER_IPV6:
//           strtype = _("prefer ipv6");
//           break;
//         }

//       char buf[disknet_grub_net_MAX_STR_ADDR_LEN];
//       disknet_grub_net_addr_to_str (&dns_servers[i], buf);
//       grub_printf ("%s (%s)\n", buf, strtype);
//     }
//   return GRUB_ERR_NONE;
// }
...
// {
//   grub_err_t err;
//   struct disknet_grub_net_network_level_address server;

//   if ((argc < 1) || (argc > 2))
//     return grub_error (GRUB_ERR_BAD_ARGUMENT, N_("one argument expected"));
//   else if (argc == 1)
//     server.option = DISKNET_DNS_OPTION_PREFER_IPV4;
//   else
//     {
//       if (grub_strcmp (args[1], "--only-ipv4") == 0)
//           server.option = DISKNET_DNS_OPTION_IPV4;
//       else if (grub_strcmp (args[1], "--only-ipv6") == 0)
//           server.option = DISKNET_DNS_OPTION_IPV6;
//       else if (grub_strcmp (args[1], "--prefer-ipv4") == 0)
//           server.option = DISKNET_DNS_OPTION_PREFER_IPV4;
//       else if (grub_strcmp (args[1], "--prefer-ipv6") == 0)
//           server.option = DISKNET_DNS_OPTION_PREFER_IPV6;
//       else
//         return grub_error (GRUB_ERR_BAD_ARGUMENT, N_("invalid argument"));
//     }

//   err = disknet_grub_net_resolve_address (args[0], &server);
//   if (err)
//     return err;

//   return disknet_grub_net_add_dns_server (&server);
// }
...
//   if (argc != 1)
//     return grub_error (GRUB_ERR_BAD_ARGUMENT, N_("one argument expected"));
//   err = disknet_grub_net_resolve_address (args[1], &server);
//   if (err)
//     return err;
