...
#define TCP_SYN_RETRANSMISSION_TIMEOUT disknet_grub_net_INTERVAL
#define TCP_SYN_RETRANSMISSION_COUNT disknet_grub_net_TRIES
#define TCP_RETRANSMISSION_TIMEOUT disknet_grub_net_INTERVAL
#define TCP_RETRANSMISSION_COUNT disknet_grub_net_TRIES
...
{
  struct disknet_grub_net_tcp_socket *next;
  struct disknet_grub_net_tcp_socket **prev;
  ...
  grub_err_t (*recv_hook) (disknet_grub_net_tcp_socket_t sock, struct disknet_grub_net_buff *nb,
			   void *recv);
  void (*error_hook) (disknet_grub_net_tcp_socket_t sock, void *recv);
  void (*fin_hook) (disknet_grub_net_tcp_socket_t sock, void *recv);
  void *hook_data;
...
{
  struct disknet_grub_net_tcp_listen *next;
  struct disknet_grub_net_tcp_listen **prev;
...
static struct disknet_grub_net_tcp_socket *tcp_sockets;
static struct disknet_grub_net_tcp_listen *tcp_listens;
...
disknet_grub_net_tcp_listen (grub_uint16_t port,
		     ...
		     void *hook_data)
{
  disknet_grub_net_tcp_listen_t ret;
...
static inline void
disknet_tcp_socket_register (disknet_grub_net_tcp_socket_t sock)
{
...
static void
error (disknet_grub_net_tcp_socket_t sock)
{
  ...
  for (unack = sock->unack_first; unack; unack = next)
    {
      next = unack->next;
      disknet_grub_netbuff_free (unack->nb);
      grub_free (unack);
    }
...
static grub_err_t
disknet_tcp_send (struct disknet_grub_net_buff *nb, disknet_grub_net_tcp_socket_t socket)
{
  grub_err_t err;
  grub_uint8_t *nbd;
  struct unacked *unack;
  struct disknet_tcphdr *tcph;
  grub_size_t size;
  tcph = (struct disknet_tcphdr *) nb->data;
  ...
  tcph->checksum = 0;
  tcph->checksum = disknet_grub_net_ip_transport_checksum (nb, disknet_grub_net_IP_TCP,
						   &socket->inf->address,
						   &socket->out_nla);
  nbd = nb->data;
  if (size)
  ...
  err = disknet_grub_net_send_ip_packet (socket->inf, &(socket->out_nla),
				 &(socket->ll_target_addr), nb,
				 disknet_grub_net_IP_TCP);
  if (err)
    return err;
  nb->data = nbd;
  if (!size)
...
void
disknet_grub_net_tcp_close (disknet_grub_net_tcp_socket_t sock,
		    int discard_received)
{
  struct disknet_grub_net_buff *nb_fin;
  struct disknet_tcphdr *tcph_fin;
  grub_err_t err;

  if (discard_received != disknet_grub_net_TCP_CONTINUE_RECEIVING)
    {
      sock->recv_hook = NULL;
  ...
  if (discard_received == disknet_grub_net_TCP_ABORT)
    sock->i_reseted = 1;
  ...
  nb_fin = disknet_grub_netbuff_alloc (sizeof (*tcph_fin)
			       + disknet_grub_net_OUR_MAX_IP_HEADER_SIZE
			       + disknet_grub_net_MAX_LINK_HEADER_SIZE);
  if (!nb_fin)
    return;
  err = disknet_grub_netbuff_reserve (nb_fin, disknet_grub_net_OUR_MAX_IP_HEADER_SIZE
			       + disknet_grub_net_MAX_LINK_HEADER_SIZE);
  if (err)
    {
      disknet_grub_netbuff_free (nb_fin);
      grub_dprintf ("net", "error closing socket\n");
      grub_errno = GRUB_ERR_NONE;
      return;
    }

  err = disknet_grub_netbuff_put (nb_fin, sizeof (*tcph_fin));
  if (err)
    {
      disknet_grub_netbuff_free (nb_fin);
      grub_dprintf ("net", "error closing socket\n");
  ...
  tcph_fin->urgent = 0;
  err = disknet_tcp_send (nb_fin, sock);
  if (err)
    {
      disknet_grub_netbuff_free (nb_fin);
      grub_dprintf ("net", "error closing socket\n");
...
static void
ack_real (disknet_grub_net_tcp_socket_t sock, int res)
{
  struct disknet_grub_net_buff *nb_ack;
  struct disknet_tcphdr *tcph_ack;
  grub_err_t err;

  nb_ack = disknet_grub_netbuff_alloc (sizeof (*tcph_ack) + 128);
  if (!nb_ack)
    return;
  err = disknet_grub_netbuff_reserve (nb_ack, 128);
  if (err)
    {
      disknet_grub_netbuff_free (nb_ack);
      grub_dprintf ("net", "error closing socket\n");
      grub_errno = GRUB_ERR_NONE;
      return;
    }

  err = disknet_grub_netbuff_put (nb_ack, sizeof (*tcph_ack));
  if (err)
    {
      disknet_grub_netbuff_free (nb_ack);
      grub_dprintf ("net", "error closing socket\n");
  ...
  err = disknet_tcp_send (nb_ack, sock);
  if (err)
    {
...
static void
ack (disknet_grub_net_tcp_socket_t sock)
{
  ack_real (sock, 0);
}

static void
reset (disknet_grub_net_tcp_socket_t sock)
{
  ack_real (sock, 1);
}
...
disknet_grub_net_tcp_retransmit (void)
{
  ...
  FOR_TCP_SOCKETS (sock)
  {
	...
	nbd = unack->nb->data;
	tcph = (struct disknet_tcphdr *) nbd;

	if ((tcph->flags & grub_cpu_to_be16_compile_time (TCP_ACK))
	    && tcph->ack != grub_cpu_to_be32 (sock->their_cur_seq))
	  {
	    tcph->checksum = 0;
	    tcph->checksum = disknet_grub_net_ip_transport_checksum (unack->nb,
							     disknet_grub_net_IP_TCP,
							     &sock->inf->address,
							     &sock->out_nla);
	  }

	err = disknet_grub_net_send_ip_packet (sock->inf, &(sock->out_nla),
				...
				const disknet_grub_net_network_level_address_t *dst)
{
  grub_uint16_t a, b = 0;
  grub_uint32_t c;
  a = ~grub_be_to_cpu16 (disknet_grub_net_ip_chksum ((void *) nb->data,
					     nb->tail - nb->data));
  ...
  switch (dst->type)
	...
	ph.proto = proto;
	b = ~grub_be_to_cpu16 (disknet_grub_net_ip_chksum ((void *) &ph, sizeof (ph)));
	break;
      }
...
cmp (const void *a__, const void *b__)
{
  ...
  struct disknet_tcphdr *a = (struct disknet_tcphdr *) a_->data;
  struct disknet_tcphdr *b = (struct disknet_tcphdr *) b_->data;
...
static void
destroy_pq (disknet_grub_net_tcp_socket_t sock)
{
  struct disknet_grub_net_buff **nb_p;
  while ((nb_p = grub_priority_queue_top (sock->pq)))
    {
      disknet_grub_netbuff_free (*nb_p);
		     ...
		     void *hook_data)
{
  ...
  err = disknet_grub_net_route_address (sock->out_nla, &gateway, &inf);
  if (err)
    return err;

  err = disknet_grub_net_link_layer_resolve (sock->inf, &gateway, &(sock->ll_target_addr));
  if (err)
    return err;

  nb_ack = disknet_grub_netbuff_alloc (sizeof (*tcph)
			       + disknet_grub_net_OUR_MAX_IP_HEADER_SIZE
			       + disknet_grub_net_MAX_LINK_HEADER_SIZE);
  if (!nb_ack)
    return grub_errno;
  err = disknet_grub_netbuff_reserve (nb_ack, disknet_grub_net_OUR_MAX_IP_HEADER_SIZE
			      + disknet_grub_net_MAX_LINK_HEADER_SIZE);
  if (err)
    {
      disknet_grub_netbuff_free (nb_ack);
  ...
  err = disknet_grub_netbuff_put (nb_ack, sizeof (*tcph));
  if (err)
    {
      disknet_grub_netbuff_free (nb_ack);
      return err;
  ...
  tcph->urgent = 0;
  sock->established = 1;
  disknet_tcp_socket_register (sock);
  err = disknet_tcp_send (nb_ack, sock);
  if (err)
    return err;
  sock->my_cur_seq++;
		   ...
		   void *hook_data)
{
  ...
  struct disknet_grub_net_network_level_interface *inf;
  disknet_grub_net_network_level_address_t gateway;
  disknet_grub_net_tcp_socket_t socket;
  ...
  grub_uint8_t *nbd;
  disknet_grub_net_link_level_address_t ll_target_addr;
  err = disknet_grub_net_resolve_address (server, &addr);
  if (err)
    return NULL;
  ...
  nb = disknet_grub_netbuff_alloc (sizeof (*tcph) + 128);
  if (!nb)
    {
      grub_free (socket);
      return NULL;
    }
  err = disknet_grub_netbuff_reserve (nb, 128);
  if (err)
    {
      grub_free (socket);
      disknet_grub_netbuff_free (nb);
  ...
  err = disknet_grub_netbuff_put (nb, sizeof (*tcph));
  if (err)
    {
      grub_free (socket);
      disknet_grub_netbuff_free (nb);
  ...
  socket->pq = grub_priority_queue_new (sizeof (struct disknet_grub_net_buff *), cmp);
  if (!socket->pq)
    {
      grub_free (socket);
      disknet_grub_netbuff_free (nb);
  ...
  tcph->checksum = disknet_grub_net_ip_transport_checksum (nb, disknet_grub_net_IP_TCP,
						   &socket->inf->address,
						   &socket->out_nla);
  disknet_tcp_socket_register (socket);
  nbd = nb->data;
  for (i = 0; i < TCP_SYN_RETRANSMISSION_COUNT; i++)
	...
	disknet_grub_net_poll_cards (500, &socket->established);
...
disknet_grub_net_send_tcp_packet (const disknet_grub_net_tcp_socket_t socket,
			  struct disknet_grub_net_buff *nb, int push)
{
  struct disknet_tcphdr *tcph;
  grub_err_t err;
  grub_ssize_t fraglen;
  COMPILE_TIME_ASSERT (sizeof (struct disknet_tcphdr) == disknet_grub_net_TCP_HEADER_SIZE);
  if (socket->out_nla.type == disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV4) {
    fraglen = (socket->inf->card->mtu - disknet_grub_net_OUR_IPV4_HEADER_SIZE
	       - sizeof (*tcph));
  }
  else {
    fraglen = 1280 - disknet_grub_net_OUR_IPV6_HEADER_SIZE;
  }
  while (nb->tail - nb->data > fraglen)
    {
      struct disknet_grub_net_buff *nb2;
      nb2 = disknet_grub_netbuff_alloc (fraglen + sizeof (*tcph)
				+ disknet_grub_net_OUR_MAX_IP_HEADER_SIZE
				+ disknet_grub_net_MAX_LINK_HEADER_SIZE);
      if (!nb2){
        return grub_errno;
	...
	return err;
      ...
      err = disknet_grub_netbuff_put (nb2, sizeof (*tcph));
      if (err) {
	return err;
      }

      tcph = (struct disknet_tcphdr *) nb2->data;
	...
	: 0;
      tcph->urgent = 0;
      err = disknet_grub_netbuff_put (nb2, fraglen);
      if (err) {
	return err;
      }
      grub_memcpy (tcph + 1, nb->data, fraglen);
      err = disknet_grub_netbuff_pull (nb, fraglen);
      if (err){
	return err;

      }
      err = disknet_tcp_send (nb2, socket);
      if (err) {
	return err;
      }
    }
  err = disknet_grub_netbuff_push (nb, sizeof (*tcph));
  if (err) {
    return err;
  }
  ...
  tcph->urgent = 0;
  return disknet_tcp_send (nb, socket);
}
			  ...
			  const disknet_grub_net_network_level_address_t *source)
{
  ...
  FOR_TCP_SOCKETS (sock)
  {
	...
	chk = tcph->checksum;
	tcph->checksum = 0;
	expected = disknet_grub_net_ip_transport_checksum (nb, disknet_grub_net_IP_TCP,
						   &sock->out_nla,
						   &sock->inf->address);
	if (expected != chk)
	  {
	...
	for (unack = sock->unack_first; unack; unack = next)
	  {
	    grub_uint32_t seqnr;
	    struct disknet_tcphdr *unack_tcph;
	    next = unack->next;
	    seqnr = grub_be_to_cpu32 (((struct disknet_tcphdr *) unack->nb->data)
				      ->seqnr);
	    unack_tcph = (struct disknet_tcphdr *) unack->nb->data;
      ...
      while (1)
	{
	  ...
	  if (!nb_top_p)
	    return GRUB_ERR_NONE;
	  nb_top = *nb_top_p;
	  tcph = (struct disknet_tcphdr *) nb_top->data;
	  if (grub_be_to_cpu32 (tcph->seqnr) >= sock->their_cur_seq)
	    break;
      ...
      while (1)
	{
	  ...
	  err = disknet_grub_netbuff_pull (nb_top, (grub_be_to_cpu16 (tcph->flags)
					    >> 12) * sizeof (grub_uint32_t));
	  if (err)
	    {
	      disknet_grub_netbuff_free (nb_top);
	  ...
	  if ((nb_top->tail - nb_top->data) > 0)
	    {
	      disknet_grub_net_put_packet (&sock->packs, nb_top);
	      do_ack = 1;
	    }
	  else
	    disknet_grub_netbuff_free (nb_top);
      ...
      while (sock->packs.first)
	{
	  ...
	  if (sock->recv_hook)
	    ...
	    disknet_grub_netbuff_free (nb);
	  disknet_grub_net_remove_packet (sock->packs.first);
	}
  ...
  if (grub_be_to_cpu16 (tcph->flags) & TCP_SYN)
    {
      disknet_grub_net_tcp_listen_t listen;
	...
	sock->pq = grub_priority_queue_new (sizeof (struct disknet_grub_net_buff *),
					    cmp);
	if (!sock->pq)
	  {
	    grub_free (sock);
