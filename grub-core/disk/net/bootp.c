...
static const void *
disknet_find_dhcp_option (const struct disknet_grub_net_bootp_packet *bp, grub_size_t size,
		  grub_uint8_t opt_code, grub_uint8_t *opt_len)
{
  ...
  if (ptr[0] != disknet_grub_net_BOOTP_RFC1048_MAGIC_0
      || ptr[1] != disknet_grub_net_BOOTP_RFC1048_MAGIC_1
      || ptr[2] != disknet_grub_net_BOOTP_RFC1048_MAGIC_2
      || ptr[3] != disknet_grub_net_BOOTP_RFC1048_MAGIC_3)
    return NULL;
  ...
  while (i < size)
    {
      ...
      /* Pad tag.  */
      if (tagtype == disknet_grub_net_BOOTP_PAD)
	continue;

      /* End tag.  */
      if (tagtype == disknet_grub_net_BOOTP_END)
	{
	  end = 1;
	  break;
	}
      ...
      if (tagtype == opt_code)
	{
      ...
      if (tagtype == disknet_grub_net_DHCP_OVERLOAD && taglength == 1)
	overload = ptr[i];
				...
				int is_def, char **device, char **path)
{
  ...
  disknet_grub_net_link_level_address_t hwaddr;
  ...
  grub_memcpy (hwaddr.mac, bp->mac_addr,
	       ...
	       : sizeof (hwaddr.mac));
  hwaddr.type = disknet_grub_net_LINK_LEVEL_PROTOCOL_ETHERNET;

  inter = disknet_grub_net_add_addr (name, card, &addr, &hwaddr, flags);
  if (!inter)
    return 0;

  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_DHCP_OVERLOAD, &opt_len);
  if (opt && opt_len == 1)
    overload = *opt;

  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_DHCP_TFTP_SERVER_NAME, &opt_len);
  if (opt && opt_len)
    {
  ...
  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_DHCP_BOOTFILE_NAME, &opt_len);
  if (opt && opt_len)
    {
  ...
  if (bp->server_ip)
    {
      ...
      disknet_grub_env_set_net_property (name, "next_server", server_ip, sizeof (server_ip));
      grub_print_error ();
    }

  if (is_def)
    disknet_grub_net_default_server = 0;
  if (is_def && !disknet_grub_net_default_server && bp->server_ip)
    {
      disknet_grub_net_default_server = grub_strdup (server_ip);
      grub_print_error ();
    }
  ...
  if (server_name)
    {
      disknet_grub_env_set_net_property (name, "dhcp_server_name", server_name, server_name_len);
      if (is_def && !disknet_grub_net_default_server)
	{
	  disknet_grub_net_default_server = grub_strdup (server_name);
	  grub_print_error ();
	}
      if (device && !*device)
	{
  ...
  if (boot_file)
    {
      disknet_grub_env_set_net_property (name, "boot_file", boot_file, boot_file_len);
      if (path)
	{
  ...
  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_BOOTP_NETMASK, &opt_len);
  if (opt && opt_len == 4)
    {
      int i;
      for (i = 0; i < 32; i++)
  ...
  disknet_grub_net_add_ipv4_local (inter, mask);

  /* We do not implement dead gateway detection and the first entry SHOULD
     be preferred one */
  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_BOOTP_ROUTER, &opt_len);
  if (opt && opt_len && !(opt_len & 3))
    {
      ...
      target.type = disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV4;
      target.ipv4.base = 0;
      target.ipv4.masksize = 0;
      gw.type = disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV4;
      gw.ipv4 = grub_get_unaligned32 (opt);
      rname = grub_xasprintf ("%s:default", name);
      if (rname)
	disknet_grub_net_add_route_gw (rname, target, gw, 0);
      grub_free (rname);
    }

  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_BOOTP_DNS, &opt_len);
  if (opt && opt_len && !(opt_len & 3))
    {
      int i;
      for (i = 0; i < opt_len / 4; i++)
	{
	  struct disknet_grub_net_network_level_address s;

	  s.type = disknet_grub_net_NETWORK_LEVEL_PROTOCOL_IPV4;
	  s.ipv4 = grub_get_unaligned32 (opt);
	  s.option = DISKNET_DNS_OPTION_PREFER_IPV4;
	  disknet_grub_net_add_dns_server (&s);
	  opt += 4;
	}
    }

  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_BOOTP_HOSTNAME, &opt_len);
  if (opt && opt_len)
    disknet_grub_env_set_net_property (name, "hostname", (const char *) opt, opt_len);

  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_BOOTP_DOMAIN, &opt_len);
  if (opt && opt_len)
    disknet_grub_env_set_net_property (name, "domain", (const char *) opt, opt_len);

  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_BOOTP_ROOT_PATH, &opt_len);
  if (opt && opt_len)
    disknet_grub_env_set_net_property (name, "rootpath", (const char *) opt, opt_len);

  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_BOOTP_EXTENSIONS_PATH, &opt_len);
  if (opt && opt_len)
    disknet_grub_env_set_net_property (name, "extensionspath", (const char *) opt, opt_len);

  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_BOOTP_CLIENT_ID, &opt_len);
  if (opt && opt_len)
    disknet_grub_env_set_net_property (name, "clientid", (const char *) opt, opt_len);

  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_BOOTP_CLIENT_UUID, &opt_len);
  if (opt && opt_len == 17)
    {
      ...
      disknet_grub_env_set_net_property (name, "clientuuid", (char *) val, 2 * opt_len + 4);
      grub_free (val);
    }
...
static grub_err_t
disknet_send_dhcp_packet (struct disknet_grub_net_network_level_interface *iface)
{
  grub_err_t err;
  struct disknet_grub_net_bootp_packet *pack;
  struct grub_datetime date;
  grub_int64_t t = 0;
  struct disknet_grub_net_buff *nb;
  struct disknet_udphdr *udph;
  disknet_grub_net_network_level_address_t target;
  disknet_grub_net_link_level_address_t ll_target;

  static struct grub_dhcp_discover_options discover_options =
    {
      {
	disknet_grub_net_BOOTP_RFC1048_MAGIC_0,
	disknet_grub_net_BOOTP_RFC1048_MAGIC_1,
	disknet_grub_net_BOOTP_RFC1048_MAGIC_2,
	disknet_grub_net_BOOTP_RFC1048_MAGIC_3,
      },
      {
	disknet_grub_net_DHCP_MESSAGE_TYPE,
	sizeof (discover_options.message_type.data),
	GRUB_DHCP_MESSAGE_DISCOVER,
      },
      disknet_grub_net_BOOTP_END,
    };
  ...
  static struct grub_dhcp_request_options request_options =
	...
	GRUB_DHCP_MESSAGE_REQUEST,
      ...
      {
	disknet_grub_net_DHCP_SERVER_IDENTIFIER,
	sizeof (request_options.server_identifier.data),
	0,
      },
      {
	disknet_grub_net_DHCP_REQUESTED_IP_ADDRESS,
	sizeof (request_options.requested_ip.data),
	0,
      },
      {
	disknet_grub_net_DHCP_PARAMETER_REQUEST_LIST,
	sizeof (request_options.parameter_request.data),
	{
	  disknet_grub_net_BOOTP_NETMASK,
	  disknet_grub_net_BOOTP_ROUTER,
	  disknet_grub_net_BOOTP_DNS,
	  disknet_grub_net_BOOTP_DOMAIN,
	  disknet_grub_net_BOOTP_HOSTNAME,
	  disknet_grub_net_BOOTP_ROOT_PATH,
	  disknet_grub_net_BOOTP_EXTENSIONS_PATH,
	},
      },
      disknet_grub_net_BOOTP_END,
    };
  ...
  nb = disknet_grub_netbuff_alloc (sizeof (*pack) + GRUB_BOOTP_MAX_OPTIONS_SIZE + 128);
  if (!nb)
    return grub_errno;

  err = disknet_grub_netbuff_reserve (nb, sizeof (*pack) + GRUB_BOOTP_MAX_OPTIONS_SIZE + 128);
  if (err)
    goto out;

  err = disknet_grub_netbuff_push (nb, GRUB_BOOTP_MAX_OPTIONS_SIZE);
  if (err)
    goto out;
  ...
  err = disknet_grub_netbuff_push (nb, sizeof (*pack));
  if (err)
    goto out;
  ...
  disknet_grub_netbuff_push (nb, sizeof (*udph));
  ...
  err = disknet_grub_net_link_layer_resolve (iface, &target, &ll_target);
  if (err)
    goto out;
  ...
  err = disknet_grub_net_send_ip_packet (iface, &target, &ll_target, nb,
...
/*
 * This is called directly from net/ip.c:disknet_handle_dgram(), because those
...
{
  char *name;
  struct disknet_grub_net_card *card = iface->card;
  const struct disknet_grub_net_bootp_packet *bp = (const struct disknet_grub_net_bootp_packet *) nb->data;
  grub_size_t size = nb->tail - nb->data;
  const grub_uint8_t *opt;
  grub_uint8_t opt_len, type;
  grub_uint32_t srv_id = 0;

  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_DHCP_MESSAGE_TYPE, &opt_len);
  if (opt && opt_len == 1)
    type = *opt;
  else
    type = GRUB_DHCP_MESSAGE_UNKNOWN;

  opt = disknet_find_dhcp_option (bp, size, disknet_grub_net_DHCP_SERVER_IDENTIFIER, &opt_len);
  ...
  if ((!iface->srv_id && type == GRUB_DHCP_MESSAGE_UNKNOWN)
      ...
      if (!name)
	{
      ...
      disknet_grub_net_configure_by_dhcp_ack (name, card, 0, bp, size, 0, 0, 0);
      grub_free (name);
      if (grub_errno)
	grub_print_error ();
      else
	disknet_grub_net_network_level_interface_unregister (iface);
    }
...
//   /* This duplicates check in disknet_find_dhcp_option to preserve previous error return */
//   if (inter->dhcp_acklen < OFFSET_OF (vendor, inter->dhcp_ack) + sizeof (grub_uint32_t)
//       || ptr[0] != disknet_grub_net_BOOTP_RFC1048_MAGIC_0
//       || ptr[1] != disknet_grub_net_BOOTP_RFC1048_MAGIC_1
//       || ptr[2] != disknet_grub_net_BOOTP_RFC1048_MAGIC_2
//       || ptr[3] != disknet_grub_net_BOOTP_RFC1048_MAGIC_3)
...
//   ptr = disknet_find_dhcp_option (inter->dhcp_ack, inter->dhcp_acklen, num, &taglength);
//   if (!ptr)
...
disknet_grub_cmd_bootp (void)
{
  ...
  FOR_NET_CARDS (card)
  {
    ...
    ifaces[j].address.type = disknet_grub_net_NETWORK_LEVEL_PROTOCOL_DHCP_RECV;
    grub_memcpy (&ifaces[j].hwaddress, &card->default_address,
		 sizeof (ifaces[j].hwaddress));
    ifaces[j].dhcp_tmo = ifaces[j].dhcp_tmo_left = 1;
    j++;
  }
  ifaces[ncards - 1].next = disknet_grub_net_network_level_interfaces;
  if (disknet_grub_net_network_level_interfaces)
    disknet_grub_net_network_level_interfaces->prev = & ifaces[ncards - 1].next;
  disknet_grub_net_network_level_interfaces = &ifaces[0];
  ifaces[0].prev = &disknet_grub_net_network_level_interfaces;
  /*
  ...
  for (i = 0; i < GRUB_DHCP_MAX_PACKET_TIMEOUT * 4; i++)
      ...
      for (j = 0; j < ncards; j++)
	{
	  ...
	  err = disknet_send_dhcp_packet (&ifaces[j]);
	  if (err)
	    {
	      grub_print_error ();
      ...
      if (!need_poll)
	break;
      disknet_grub_net_poll_cards (200, 0);
    }
  err = GRUB_ERR_NONE;
  for (j = 0; j < ncards; j++)
    {
	...
	continue;
      grub_error_push ();
      disknet_grub_net_network_level_interface_unregister (&ifaces[j]);
